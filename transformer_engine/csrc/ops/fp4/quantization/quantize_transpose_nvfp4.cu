...
constexpr size_t BUFF_DIM_Y = TILE_DIM_Y; // 16
...
constexpr size_t ITERATIONS_TRANSPOSE = BUFF_IN_DIM_Y / SCALE_DIM;    // 16 /16 = 1
...
__global__ void __launch_bounds__(THREADS_NUM)
  ...
  constexpr size_t BLOCKS_PER_TILE_Y = TILE_DIM_Y / BLOCK_DIM;  // 16/16 = 1
  constexpr size_t BLOCKS_PER_TILE_X = TILE_DIM_X / BLOCK_DIM;  // 128/16 = 8
  constexpr size_t ITERATIONS_BLOCK = BLOCKS_PER_TILE_Y;  // iterations to calculate 2d block amaxes of 1 tile
  ...
  const size_t scales_block_offset_Y_rowwise = blockIdx.y * CHUNK_DIM_Y;  // y * 128
  const size_t scales_block_offset_X_rowwise = blockIdx.x * SCALES_PER_CHUNK_X; // x * (128/16)=x*8
  const size_t scales_block_offset_Y_t = blockIdx.x * CHUNK_DIM_X;
  const size_t scales_block_offset_X_t = blockIdx.y * SCALES_PER_CHUNK_Y;

  const size_t tid_Y_rowwise = threadIdx.x / THREADS_X_ROWWISE; // [0~127]/8
  const size_t tid_X_rowwise = threadIdx.x % THREADS_X_ROWWISE; // [0~127]%8
  const size_t tid_X_colwise = threadIdx.x;
  const size_t tid_Y_t = tid_X_colwise;
    ...
    for (size_t block_iter = 0; block_iter < ITERATIONS_BLOCK; ++block_iter) {
      ...
      if constexpr (NO_ACTIVATIONS_NOT_FP32_INPUT) {
        for (int elem = 0; elem < BLOCK_DIM; elem += 2) {
          ...
          abs_max_2x(thread_amax_2x, thread_amax_2x, val_2x);
        }
...
void quantize_transpose(
  ...
  const size_t scale_stride = output_scale->stride(0);
  ...
  constexpr size_t out_scales_mem = buff_size_scales;
  size_t out_data_transpose_mem = return_transpose ? buff_size_aligned_out : 0;
  size_t out_scales_transpose_mem = return_transpose ? buff_size_scales : 0;

  size_t out_mem = out_data_mem + out_data_transpose_mem;

  size_t dshmem_size = in_mem + out_mem + out_scales_mem + out_scales_transpose_mem + TMA_SHMEM_ALIGNMENT;
  ...
  TRANSFORMER_ENGINE_SWITCH_CONDITION(
      ...
      TRANSFORMER_ENGINE_SWITCH_CONDITION(return_transpose, RETURN_TRANSPOSE, {
        ...
        kernel<<<grid, block_size, dshmem_size, stream>>>(
            ...
            scales_transpose_ptr, rows, cols,
